...
using Fitness.Models.DataAccessLayer.Footer;


namespace Fitness.Controllers.Footer
{
    //[Authorize]
    [RoutePrefix("api/footerDetails")]
    public class mstrFooterDetailsController : ApiController
    {
        readonly DAL_MstrFooterDetails DAL_MstrFooterDetails = new DAL_MstrFooterDetails();

        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetFooterDetailsAsync([FromUri] GetFooter_In Input)
        {
            try
            {

                var listOfGetFooter_Out = await DAL_MstrFooterDetails.UD_GetFooterAsync(Input);

                return GetSuccessOrFailureResForSelect<GetFooter_Out>(listOfGetFooter_Out, "No Records Found In Footer Details Master !!!");
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [Route("insert")]
        public async Task<IHttpActionResult> InsertFooterDetailsAsync([FromBody] InsertFooter_In Input)
        {
            try
            {
                ...
                DB_Response DB_res = await DAL_MstrFooterDetails.UD_InsertMstrFooterDetailsAsync(Input);
        ...
        [HttpPost]
        [Route("update")]
        public async Task<IHttpActionResult> UpdateFooterAsync([FromBody] UpdateFooter_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrFooterDetails.UD_UpdateMstrFooterDetailsAsync(Input);
        ...
        public async Task<IHttpActionResult> ActivateOrInactivateFooterAsync([FromBody] ActivateOrInactivateFooter_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrFooterDetails.UD_ActivateOrInactivateFoodterAsyn(Input);
                return GetDBSuccessOrInfoResponse(DB_res, "");
            }
            catch (Exception ex)
            {
