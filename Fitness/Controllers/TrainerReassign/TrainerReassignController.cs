...
using Fitness.Models.DataAccessLayer.TrainerReassign;
...
{ 
    ...
    //[Authorize]
    [RoutePrefix("api/trainerReassign")]
    public class TrainerReassignController : ApiController
    {
        readonly DAL_TrainerReassign DAL_TrainerReassign = new DAL_TrainerReassign();

        [HttpGet]
        [Route("getddlDetails")]
        public async Task<IHttpActionResult> GetTrainerDetailsAsync([FromUri]  GetAbsentTrainer_In Input)
        {
            try
            {
                var listOfGetMstrOwner_Out = await DAL_TrainerReassign.UD_GetMstrTrainerReassign(Input);

                return GetSuccessOrFailureResForJArray(listOfGetMstrOwner_Out, "No Trainer Details Found!!!");
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [Route("insert")]
        public async Task<IHttpActionResult> InsertTrainerReassignAsync([FromBody] InsertTrainerReassign_In Input)
        {
            try
            {
                ...
                DB_Response DB_res = await DAL_TrainerReassign.UD_InsertTrainerReassign(Input);
