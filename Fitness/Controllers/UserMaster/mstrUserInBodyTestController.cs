...
{
                  //[Authorize]
    [RoutePrefix("api/userInBodyTest")]
    public class mstrUserInBodyTestController : ApiController
    {

        readonly DAL_MstrUserInBodyTest DAL_MstrUserInBodyTest = new DAL_MstrUserInBodyTest();


        /// <summary>
        /// this method is used to get UserInBodyTest details
        /// </summary>
        /// <param name="Input">GetUserInBodyTest_In class as input parameter</param>
        /// <returns></returns>
        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetUserInBodyTestMasterAsync([FromUri] GetUserInBodyTest_In Input)
        {
            try
            {
                ...
                var listOfGetConfigMstr_Out = await DAL_MstrUserInBodyTest.UD_GetUserInBodyTestAsync(Input);

                return GetSuccessOrFailureResForJArray(listOfGetConfigMstr_Out, "No Records Found In UserInBodyTest Master !!!");
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        [Route("AllUser")]
        public async Task<IHttpActionResult> GetAllUserInBodyTestMasterAsync()
        {
            try
            {

                var listOfGetConfigMstr_Out = await DAL_MstrUserInBodyTest.UD_GetAllUserInBodyTestAsync();
        ...
        /// <summary>
        /// this method is used to insert UserInBodyTest
        /// </summary>
        /// <param name="Input">InsertUserInBodyTest_In class as input parameter</param>
        /// <returns></returns>
        [HttpPost]
        [Route("insert")]
        public async Task<IHttpActionResult> InsertUserInBodyTestMasterAsync([FromBody] InsertUserInBodyTest_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrUserInBodyTest.UD_InsertMstrUserInBodyTestAsync(Input);
        ...
        [HttpPost]
        [Route("insertDepartmemt")]
        public async Task<IHttpActionResult> InsertUserInBodyTestAsync([FromBody] InsertUserInBodyTestDep_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrUserInBodyTest.UD_InsertUserInBodyTestAsync(Input);
