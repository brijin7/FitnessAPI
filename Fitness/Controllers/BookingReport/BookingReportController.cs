...
using Fitness.Models.DataAccessLayer.BookingReport;
using System;
...
namespace Fitness.Controllers.BookingReport
{
                       //[Authorize]
    [RoutePrefix("api/bookingReport")]
    public class BookingReportController : ApiController
    {
        readonly DAL_RptBookingReport BookingRpt = new DAL_RptBookingReport();

        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> InsertBookingAsyc([FromUri] GetBookingReport_In Input)
        {
            try
            {
                ...
                var output = await BookingRpt.UD_GetBookingReport(Input);
                return GetSuccessOrFailureResForSelect<GetBookingReport_Out>(output, "No Records Found!!!");
            }
            catch (Exception ex)
            {
