...
using Fitness.Models.DataAccessLayer.EmployeeMaster;

namespace Fitness.Controllers
{
    //[Authorize]
    [RoutePrefix("api/employee")]
    public class mstrEmployeeController : ApiController
    {
        readonly DAL_MstrEmployee DAL_MstrEmployee = new DAL_MstrEmployee();


        /// <summary>
        /// this method is used to get Employee Master details
        /// </summary>
        /// <param name="Input">GetMstrEmployee_In class as input parameter</param>
        /// <returns></returns>
        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetMstrEmployeeAsync([FromUri] GetMstrEmployee_In Input)
        {
            try
            {
                ...
                var listOfGetMstrEmployee_Out = await DAL_MstrEmployee.UD_GetMstrBranchAsync(Input);

                return GetSuccessOrFailureResForSelect<GetMstrEmployee_Out>(listOfGetMstrEmployee_Out, "No Records Found In Employee Master!!!");
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("ddlEmployeeList")]
        public async Task<IHttpActionResult> ddlMstrEmployeeAsync([FromUri] ddlMstrEmployee_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                var listOfGetMstrEmployee_Out = await DAL_MstrEmployee.UD_ddlMstrBranchAsync(Input);

                return GetSuccessOrFailureResForSelect<ddlMstrEmployee_Out>(listOfGetMstrEmployee_Out, "No Records Found In Employee Master!!!");
            }
            catch (Exception ex)
            {
                return GetErrorResponse(ex);
        ...
        /// <summary>
        /// This Method Used For Insert Employee Master
        /// </summary>
        /// <param name="Input"></param>
        /// <returns></returns>
        [HttpPost]
        [Route("insert")]
        public async Task<IHttpActionResult> InsertMstrEmployeeAsyc([FromBody] InsertMstrEmployee_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrEmployee.UD_InsertMstrBranchAsyc(Input);
        ...
        [HttpPost]
        [Route("update")]
        public async Task<IHttpActionResult> UpdateMstrEmployeeAsync([FromBody] UpdateMstrEmployee_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrEmployee.UD_UpdateMstrEmployeeAsyc(Input);
        ...
        public async Task<IHttpActionResult> ActiveOrInactiveMstrEmployeeAsync([FromBody] ActivateOrInactivateMstrEmployee_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrEmployee.UD_ActivateOrInactivateMstrEmployeeAsyc(Input);
                return GetDBSuccessOrInfoResponse(DB_res, "");
            }
            catch (Exception ex)
            {
