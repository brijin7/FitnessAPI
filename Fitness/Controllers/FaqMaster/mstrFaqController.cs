...
using Fitness.Models.DataAccessLayer.FaqMaster;
using static Fitness.Models.BusinessObject.FaqMaster.BOL_FaqMaster;


namespace Fitness.Controllers.FaqMaster
{
    //[Authorize]
    [RoutePrefix("api/faqMaster")]
    public class mstrFaqController : ApiController
    {

        readonly DAL_FaqMaster DAL_FaqMaster = new DAL_FaqMaster();

        /// <summary>
        /// this method is used to get Faqmstr details
        /// </summary>
        /// <param name="Input">GetFaqMstr_In class as input parameter</param>
        /// <returns></returns>
        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetFaqMasterAsync([FromUri] GetFaqMstr_In Input)
        {
            try
            {
                ...
                var listOfGetConfigMstr_Out = await DAL_FaqMaster.UD_GetFaqMstrAsync(Input);

                return GetSuccessOrFailureResForSelect<GetFaqMstr_Out>(listOfGetConfigMstr_Out, "No Records Found In FAQ Master!!!");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// this method is used to insert Faqmstr
        /// </summary>
        /// <param name="Input">InsertFaq_In class as input parameter</param>
        /// <returns></returns>
        [HttpPost]
        [Route("insert")]
        public async Task<IHttpActionResult> InsertFaqMasterAsync([FromBody] InsertFaq_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_FaqMaster.UD_InsertFaqMasterAsync(Input);
        ...
        /// <summary>
        /// this method is used to update FaqMaster
        /// </summary>
        /// <param name="Input">UpdateFaqMstr_In class as input parameter</param>
        /// <returns></returns>
        [HttpPost]
        [Route("update")]
        public async Task<IHttpActionResult> UpdateFaqMasterAsync([FromBody] UpdateFaqMstr_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_FaqMaster.UD_UpdateFaqMstrAsync(Input);
        ...
        public async Task<IHttpActionResult> ActiveOrInactiveCategoryMasterAsync([FromBody] ActivateOrInactivateFaqMstr_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_FaqMaster.UD_ActivateOrInactivateFaqMstrAsyn(Input);
                return GetDBSuccessOrInfoResponse(DB_res, "");
            }
            catch (Exception ex)
            {
