...
using Fitness.Models.DataAccessLayer.MealTimeConfig;
namespace Fitness.Controllers.MealTimeConfig
{
    //[Authorize]
    [RoutePrefix("api/mealTimeConfig")]
    public class MealTimeConfigController : ApiController
    {
        readonly DAL_MstrMealTimeConfig DAL_MstrMealTimeConfig = new DAL_MstrMealTimeConfig();


        /// <summary>
        /// this method is used to get MealTimeConfig details
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetFoodDietTimeMasterAsync([FromUri] MealTimeConfig_In Input)
        {
            try
            {
                ...
                var listOfGetConfigMstr_Out = await DAL_MstrMealTimeConfig.UD_GetMealTimeConfigAsync(Input);

                return GetSuccessOrFailureResForSelect<MealTimeConfig_Out>(listOfGetConfigMstr_Out, "No Records Meal Time Config Master !!!");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// this method is used to insert MealTimeConfig
        /// </summary>
        /// <param name="Input">InsertMealTimeConfig_In class as input parameter</param>
        /// <returns></returns>
        [HttpPost]
        [Route("insert")]
        public async Task<IHttpActionResult> InsertFoodDietTimeMasterAsync([FromBody] InsertMealTimeConfig_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrMealTimeConfig.UD_InsertMealTimeConfigAsync(Input);
        ...
        /// <summary>
        /// this method is used to update MealTimeConfig master
        /// </summary>
        /// <param name="Input">UpdateMealTimeConfig_In class as input parameter</param>
        /// <returns></returns>
        [HttpPost]
        [Route("update")]
        public async Task<IHttpActionResult> UpdateFoodDietTimeMasterAsync([FromBody] UpdateMealTimeConfig_In Input)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                DB_Response DB_res = await DAL_MstrMealTimeConfig.UD_UpdateMealTimeConfigAsync(Input);
